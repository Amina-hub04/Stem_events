...
1. **`scraper.py`** — pulls raw visible text off each source page
   (`requests` + BeautifulSoup). If a page looks JS-rendered (very little
   text comes back), it falls back to Playwright to load the page in a
   real browser.
2. **`ai_extract.py`** — sends that raw text to Groq (Llama 3.3 70B) with one extraction
   prompt that works across every site's different layout, and gets back
   structured JSON: title, date, location, category, source. This is the
   part that replaces writing a custom HTML parser per site.
3. **`filters.py`** — keeps only STEM-tagged events whose date falls in
   the target window (today - 30 days to today + 60 days), and de-dupes.
...
`main.py` daily and commits the refreshed `events_output.json` straight
back to the repo. To activate it:

1. Push this project to a GitHub repo.
2. In the repo, go to **Settings → Secrets and variables → Actions → New
   repository secret**, add a secret named `GROQ_API_KEY` with your key
   as the value. (Never commit the key itself to the repo.)
3. That's it — the workflow runs automatically every day at 06:00 UTC.
   You can also trigger it manually any time from the **Actions** tab (it
   has `workflow_dispatch` enabled).
...
Just add more entries to `SOURCES` in `config.py` — e.g. 10times.com
global tech listings, Eventbrite's tech category, etc. Nothing else in
the pipeline needs to change, since the AI extraction step adapts to
each page's format automatically.
...
- Groq free tier has rate limits — if you're hitting many sources, add
  a short `time.sleep()` between calls (already built into
  `ai_extract.extract_events`, adjustable via `sleep_between_calls`).
